...
function atualizarValor() {
  ...
  // Adicionais pagos
  document.querySelectorAll('input[name="adicionais"]:checked').forEach(ad => {
...
//
 ...
 function toggleTipo() {
  ...
  document.querySelectorAll('input[name="complementos"]').forEach(input => input.checked = false)
  document.querySelectorAll('input[name="tamanho"]').forEach(input => input.checked = false)
  document.querySelectorAll('input[name="adicionais"]').forEach(input => input.checked = false)
...
function limitarComplementos() {
  ...
  const selecionados = Array.from(document.querySelectorAll('input[name="complementos"]:checked'))
...
function adicionarPedido() {
  ...
  const tipo = document.getElementById('tipoPedido').value
  const tamanho = document.querySelector('input[name="tamanho"]:checked')
  const complementos = Array.from(document.querySelectorAll('input[name="complementos"]:checked')).map(c => c.value)
  const adicionais = Array.from(document.querySelectorAll('input[name="adicionais"]:checked')).map(a => a.value)
...
function limparFormulario() {
  ...
  document.querySelectorAll('input[name="tamanho"], input[name="complementos"], input[name="adicionais"]').forEach(input => input.checked = false)
  document.querySelector('textarea[name="observacao"]').value = ''
  document.getElementById('valorTotal').innerText = '0.00'
}
